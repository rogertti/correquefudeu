<?php
    ...
    $msg = "Campo obrigat&oacute;rio vazio.";
        ...
        if(empty($_POST['idsobre'])) {die("Vari&aacute;vel de controle nula."); }
        if(empty($_FILES['midia'])) { $midia = $_POST['img']; $filtro = 1; } else {
            $filtro = 1;
        ...
        if($filtro == 2) {
            try {
                ...
                $sql = $pdo->prepare("UPDATE sobre SET midia = :midia,texto = :texto WHERE idsobre = :idsobre");
                $sql->bindParam(':midia', $midia, PDO::PARAM_STR);
                $sql->bindParam(':texto', $_POST['texto'], PDO::PARAM_STR);
                $sql->bindParam(':idsobre', $_POST['idsobre'], PDO::PARAM_INT);
                $res = $sql->execute();
